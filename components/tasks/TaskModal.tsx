...
export default function TaskModal({ open, onClose, onSave, initial, mode = "create" }: TaskModalProps) {
  ...
  return (
    <Modal open={open} onClose={onClose} title={mode === "create" ? "Nouvelle tâche" : "Modifier la tâche"}>
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <div className="flex gap-2">
            {[1, 2, 3, 4, 5].map((n) => (
              ...
              >
                <svg
                  className={`w-6 h-6 mx-auto transition-colors ${n <= form.importance ? "text-amber-400" : "text-gray-200"}`}
                  fill="currentColor"
                  viewBox="0 0 20 20"
                >
        ...
        {/* Deadline */}
        <div>
          <label className="text-xs font-medium text-gray-500 block mb-1.5">Deadline (optionnel)</label>
          <input
            type="date"
            value={form.deadline}
        ...
        <label className="flex items-center gap-3 cursor-pointer">
          <div
            ...
            className={`w-10 h-5 rounded-full transition-colors relative ${form.isSimpleTodo ? "bg-orange-500" : "bg-gray-200"}`}
          >
